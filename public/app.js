...
function showResult(winner, loser) {
  ...
  const title = document.createElement("strong");
  const stats = document.createElement("span");
  title.textContent = `${winner.name} wins this pair`;
...
async function submitVote(winner, loser) {
  ...
  photos = data.photos;
  const updatedWinner = photos.find((photo) => photo.id === winner.id);
  const updatedLoser = photos.find((photo) => photo.id === loser.id);
